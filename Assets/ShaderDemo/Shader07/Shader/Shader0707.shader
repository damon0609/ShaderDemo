    ...
    SubShader
    {
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float3 viewDir = normalize(UnityWorldSpaceViewDir(i.worldVertex));//观察方向的

                float3 halfDir = normalize(viewDir+lightDir);
                float3 normal = normalize(i.worldNormal);

                float3 specular = _LightColor0.rgb*_Specular.rgb*pow(saturate(dot(normal,halfDir)),_Gloss);
